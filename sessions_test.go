...
func TestFlashes(t *testing.T) {
	...
	rsp = NewRecorder()
	// Get a session.
	if session, err = store.Get(req.Context(), "session-key"); err != nil {
	...
	if _, err = store.Get(req.Context(), "session:key"); err.Error() != "sessions: invalid character in cookie name: session:key" {
